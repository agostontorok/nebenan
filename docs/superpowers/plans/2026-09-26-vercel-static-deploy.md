...
Expected: the two `build_script`/`executable` tests PASS; the rest still fail.
...
Run: `.venv/bin/python -m pytest tests/test_static_deploy.py -q -k "excluded or ignore_file"`
Expected: the three `.vercelignore` tests PASS.
